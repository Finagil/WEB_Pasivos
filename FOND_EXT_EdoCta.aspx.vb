    ...
    Protected Sub GridView1_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If Cap > 0 Then
                ...
                e.Row.Cells(9).Text = EncuentraBaseFOR(Cap, Factor, Rete, 0.1).ToString("n2") ' base
                e.Row.Cells(10).Text = Math.Abs(Factor)
            End If
            e.Row.Cells(12).Text = CDec(taEdoCta.CapitalPagado(ID, FecFin.Month, FecFin.Year)).ToString("n2")
            e.Row.Cells(13).Text = CDec(taEdoCta.InteresPagado(ID, FecFin.Month, FecFin.Year)).ToString("n2")
            e.Row.Cells(14).Text = CDec(taEdoCta.RetencionPagada(ID, FecFin.Month, FecFin.Year)).ToString("n2")
            Pago = CDec(e.Row.Cells(13).Text)
            e.Row.Cells(15).Text = Math.Abs(CapFinal + Inte + InteAcum + Pago).ToString("n2") ' Saldo Neto
            InteAcum = CDec(e.Row.Cells(15).Text) - CapFinal
